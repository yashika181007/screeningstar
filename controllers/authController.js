...
const generatePassword = (length = 8) => {
  if (length < 8) {
    throw new Error(
      "Password length should be at least 8 to meet the requirements."
    );
  }
...
exports.createuser = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Employee registered successfully", user: newUser });
  } catch (error) {
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const usersWithPhotos = users.map((user) => ({
      ...user.dataValues,
      employeePhoto: `${req.protocol}://${req.get("host")}/uploads/${
        user.employeePhoto
      }`,
    }));
...
exports.changeUserStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error changing user status", error: err.message });
  }
};

...
exports.createpackage = async (req, res) => {
  try {
    ...
    if (!packageName || !packageDescription) {
      return res
        .status(400)
        .json({ message: "Package name and description are required" });
    }
...
exports.updatepackage = async (req, res) => {
  ...
  try {
    ...
    foundPackage.packageDescription =
      packageDescription || foundPackage.packageDescription;

    await foundPackage.save();
    res
      .status(200)
      .json({ message: "Package updated successfully", package: foundPackage });
  } catch (error) {
